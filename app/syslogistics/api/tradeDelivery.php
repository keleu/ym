...
class syslogistics_api_tradeDelivery {
    ...
    public $apiDescription = "商户线下发货接口, 一次性全部商品发货";
    ...
    public function getParams()
    {
        $return['params'] = array(
            ...
            'shop_id' =>['type'=>'int','valid'=>'required', 'description'=>'店铺ID','default'=>'','example'=>'18'],
            'seller_id' =>['type'=>'int','valid'=>'required', 'description'=>'商家ID','default'=>'','example'=>'1'],
        );
        return $return;
    }
    ...
    {
        //获取登陆用户的id以及店铺号
        $delivery['seller_id'] = $params['sellerId'];
        $delivery['shop_id'] = $params['shop_id'];

        //将请求的商品信息json格式处理为需要使用的数据结构
        $tradeInfo = array();
        $delivery['tid'] = $params['tid'];
        $delivery['items'] = $this->__getItems($params['tid'], $params['shop_id'], $params['items'], $tradeInfo);
        ...
        $delivery['template_id'] = $tradeInfo['dlytmpl_id'];
