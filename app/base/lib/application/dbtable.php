...
class base_application_dbtable extends base_application_prototype_filepath
{
    ...
    public function &load($check_lastmodified=true)
    {
        ...
        if(!static::$_define[$real_table_name])
        {
            ...
            {
                if ($check_lastmodified) $lastModified = md5_file($path);
                // kvstore中找不到 or 新格式数据不存在 or 检查是否发生变更(当 $check_lastmodified 为true时不检查)
                if ((!base_kvstore::instance('tbdefine')->fetch($this->target_app->app_id.$this->key, $store) || !$store['content']) ||
                    (!$check_lastmodified ? false : ($lastModified != $store['lastModified'])))
                {
                    $define = $this->loadDefine($path);
                    $store = ['lastModified' => $lastModified,
                               'content' => $define];

                    base_kvstore::instance('tbdefine')->store($this->target_app->app_id.$this->key, $store);
                }
                
                $define = $store['content'];
            }
