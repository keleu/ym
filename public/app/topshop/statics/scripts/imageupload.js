...
jQuery(function(){
        ...
        if(isMultiple && isMultiple == 'true') {
            ...
            $(imgList).each(function(index, el) {
                multipleList +=  '<div class="multiple-item">'
                                ...
                                + '<input type="hidden" name="'+ name +'" value="'+ $(el).find('.caption').attr('data-name') +'">'
                                + '<div class="img-put">'
    ...
    $('#gallery_modal').on('click','.thumbnail',function(e){
        e.preventDefault();
        var that = $(this);
        var currentId = that.find('.caption').attr('data-name');
        ...
        $(hasImgNum).each(function(index, el) {
            var checkedId = $(el).find('input[name^="list"]').val();

            if(checkedId == currentId){
                $('#messagebox').message('您已经选择了该图片!');
