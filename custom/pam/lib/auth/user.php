...
class pam_auth_user
{
    ...
    {
        //2015-12-4 by jiang 将这个判断去掉 因为不管是手机还是邮箱登录  首页都应该显示用户名
        // if (!$loginName)
        // {
        ...
        // dump($_SESSION);
        // exit;
        if($_REQUEST['state_wx'] != ''){
            $_model = app::get('sysuser')->model('user');
            // $row = $_model->getRow('user_id,opend_id', ['user_id'=>$userId]);
            
            $_openId_wx = $_SESSION['wxweb']['openId'];
            if($_openId_wx != ''){
                $del = $_model->update(['opend_id'=>''],['opend_id'=>$_openId_wx]);
                $result = $_model->update(['opend_id'=>$_openId_wx],['user_id'=>$userId]);
            }
        }
    ...
    public function signUp($loginName,$password, $confirmedPassword,$opend_id)
    {
    ...
    /**
     * ps ：手机端由代理商或子帐号开通会员
     * Time：2015/12/07 13:36:58
     * @author zhangyan
     * @param 参数类型
     * @return 返回值类型
    */
    public function signUpByApp($account_id,$loginName,$mobile,$password, $confirmedPassword)
    {
        return app::get('pam')->rpcCall('user.create',
                                         ['account' => $loginName,
                                          'mobile' => $mobile,
