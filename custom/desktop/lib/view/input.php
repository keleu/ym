...
class desktop_view_input
{
    ...
    function input_object($params){
        ...
        if($params['value']){
            ...
            $params['items'] = &$o->getList('*',array($key=>$params['value']),0,-1,$params['orderBy']);


            //添加自定义函数，该函数主要处理一些数据信息时候用到
            if($params['extendfunction']!=''){
                list($extend_ctl ,$extend_action) = explode('@',$params['extendfunction']);
                kernel::single($extend_ctl)->$extend_action($params);
            }
        ...
        if(isset($params['multiple']) && $params['multiple']){
            ...
            $url = '?app=desktop&act=alertpages&goto='.urlencode('?app=desktop&ctl=editor&act=finder_common&app_id='.$app_id.'&'.$getobject.$getdata.'&dodelete='.$params['dodelete']);
