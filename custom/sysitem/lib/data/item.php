...
class sysitem_data_item {
    ...
    public function setSaleStatus($itemId, $status="onsale")
    {
        ...
        if($status=='onsale')
        {
            ...
            if($statusInfo['approve_status'] == 'check' || $statusInfo['check_res'] == 'active')
            {
                throw new \LogicException('商品未审核完成不能上架');
            }
            if($statusInfo['check_res']=='failing')
            {
                throw new \LogicException('商品审核被拒绝不能上架');
            }
        ...
        if($status=='check')
        {
            ...
            $statusInfo = $objMdlStatus->getRow('*', array('item_id'=>$itemId));
            //审核次数+1
            $numShenhe=$statusInfo['shenhe_num']+1;
            if( $statusInfo['check_res']=='successful')
            {
                throw new \LogicException('商品已审核完成');
            }
            $data = array('approve_status' => 'check','check_res'=>'active','shenhe_num'=>$numShenhe);
        }
    ...
    {   
        ...
        if( !($postData['integral']>0 || $postData['price']>0 || !empty($postData['blend'])))
        {
            $msg = app::get('sysitem')->_('商品积分价、标准售价、混合价必填一项');
            throw new \LogicException($msg);
        }
        if( !$postData['cat_id'] )
        {
    ...
    private function __checkSku($postData)
    {
        ...
        if($postData && !$postData['spec'] && ( !reset($postData['sku']) || !$postData['sku'] ))
        {
            ...
            $postData['sku'][0]['integral'] = $postData['integral'];
            $postData['sku'][0]['blend'] = $postData['blend'];
        ...
        else
        {
            ...
            {
                $postData['sku'][$k]['integral'] = $postData['integral'];
                $postData['sku'][$k]['price'] = $postData['price'];
                $postData['sku'][$k]['blend'] = $postData['blend'];
        ...
        if(!$this->checkPriceIntegral($postData['sku']))
        {
            $msg = app::get('sysitem')->_( '规格中积分价、标准售价、混合价必填一项' );
            throw new \LogicException($msg);
        }
    ...
    function add($postData)
    {
        ...
        if( $item['blend'] )
        {
            $tempBlend=explode('+',$item['blend']);
            if(!(is_numeric($tempBlend[0])&&$tempBlend[0]>0&&is_numeric($tempBlend[1])&&$tempBlend[1]>0)){
                $msg = app::get('sysitem')->_('混合价格式错误，请检查！');
                throw new \LogicException($msg);
            }
            $item['blend_integral'] = $tempBlend[0];
            $item['blend_price'] = $tempBlend[1];
        }else{
            $item['blend_integral'] = 0;
            $item['blend_price'] = 0;
        }

        //2016-5-13 by jianghui 重量大于0
        if(!((float)$item['weight']>0)){
            $msg = app::get('sysitem')->_('重量必须大于0');
            throw new \LogicException($msg);
        }
        ...
        foreach($item['sku'] as $k => $p)
        {
            ...
            if( $p['blend'] )
            {
                $tempBlend=explode('+',$p['blend']);
                if(!(is_numeric($tempBlend[0])&&$tempBlend[0]>0&&is_numeric($tempBlend[1])&&$tempBlend[1]>0)){
                    $msg = app::get('sysitem')->_('混合价格式错误，请检查！');
                    throw new \LogicException($msg);
                }
                $item['sku'][$k]['blend_integral'] = $tempBlend[0];
                $item['sku'][$k]['blend_price'] = $tempBlend[1];
            }else{
                $item['sku'][$k]['blend_integral'] = 0;
                $item['sku'][$k]['blend_price'] = 0;
            }

            $item['sku'][$k]['price'] = $p['price']?$p['price']:0;
            $item['sku'][$k]['integral'] = $p['integral']?$p['integral']:0;
        ...
        //对比前后的积分是否发生变化
        $objMdlStatus = app::get('sysitem')->model('item');
        $statusInfo = $objMdlStatus->getRow('*', array('item_id'=>$postData['item']['item_id']));
        // 商品状态
        ...
        if($postData['item']['approve_status'] == 'instock')
        {
            ...
            if($postData['item']['integral'] != $statusInfo['integral'] && $statusInfo && $statusInfo['check_res']=='failing')
            {
                $item['list_status']['is_Adjusted'] = '1';
            }
            if($postData['item']['integral'] != $statusInfo['integral'])
            {
                $item['list_status']['check_res'] = 'active';
            }
        }
    ...
    function _prepareItemData( &$data )
    {
        ...
        foreach( $item['sku'] as $prok => $pro )
        {
            ...
            if( $pro['price'] === '' )
            {
                $item['sku'][$prok]['price'] = 0;
            }
            if( $pro['integral'] === '' )
            {
                $item['sku'][$prok]['integral'] = 0;
            }
            $item['sku'][$prok]['integral'] = trim($item['sku'][$prok]['integral']);
    ...
    /**
    * ps ：判断价格
    * Time：2016/04/01 12:03:01
    * @author jianghui
    */
    private function checkPriceIntegral($data)
    {
        if(is_array($data))
        {
            foreach($data as $key=>&$val)
            {
                if( !($val['integral']>0 || $val['price']>0 || !empty($val['blend'])))
                {
                    return false;
                }
            }
