...
class sysitem_item_info {
    ...
    public function getItemInfo($filter, $rows='*', $fields)
    {
        ...
        //2016-3-30 by jianghui 添加混合价显示
        $itemInfo['blend'] = kernel::single('sysitem_blendShow')->show($itemInfo['blend_integral'],$itemInfo['blend_price']);
    ...
    private function __searchSkuList($filter, $rows='*', $isStore=true )
    {
        ...
        if( $isStore )
        {
            foreach( $skuList as $k=>$row )
            {
                $skuIds[] = $row['sku_id'];

                //2016-3-30 by jianghui 添加混合价显示
                $skuList[$k]['blend'] = kernel::single('sysitem_blendShow')->show($row['blend_integral'],$row['blend_price']);
            }
            $objMdlSkuStore = app::get('sysitem')->model('sku_store');
