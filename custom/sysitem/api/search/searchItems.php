...
class sysitem_api_search_searchItems{
    ...
    public function getParams()
    {
        $return['params'] = array(
            ...
            //2016-4-29 by jianghui 将商店改为可以搜索多个商店
            'shop_id' => ['type'=>'string','valid'=>'','description'=>'店铺id，多个id用，隔开','example'=>'2,3,5,6','default'=>''],
    ...
    {
        $filterCols = ['item_id','shop_id','shop_cat_id','cat_id','search_keywords','approve_status','brand_id','prop_index','is_selfshop'];
        foreach( $filterCols as $col )
        {
            if( $params[$col] )
            {
                ...
                if( in_array($col,['item_id','brand_id','shop_cat_id','prop_index','shop_id']) )
                {
    ...
    public function getList($params)
    {
        ...
        //2016-5-13 by jianghui 添加判断是否分页 默认为分页
        $limit = $params['no_page']?null:$limit;
        ...
        //2016-3-29 by jianghui 混合模式显示配置
        foreach ($data['list'] as & $b) {
            $b['blend'] = kernel::single('sysitem_blendShow')->show($b['blend_integral'],$b['blend_price']);
        }
        return $data;
    }
}
