                    ...
                    if($isUpdatedPay)
                    {
                        ...
                        try
                        {
                            ...
                            if($paymentBill['status'] == "succ" || $paymentBill['status'] == "progress")
                            {
                                ...
                                {
                                    app::get('ectools')->rpcCall('trade.pay.finish',array('tid'=>$value['tid'],'payment'=>$value['payment'],'payment_integral'=>$value['payment_integral']));
                                    $temp_tids[] = $value['tid'];
                                }
                                //会员扣除积分
                                $objRefund = kernel::single('sysuser_data_point');
                                $sdf['behavior'] = '订单支付';
                                $sdf['tids'] = join(',',$temp_tids);
                                $result = $objRefund->update($sdf);
