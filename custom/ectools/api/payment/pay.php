...
class ectools_api_payment_pay{
    ...
    public function getParams()
    {
        $return['params'] = array(
            ...
            'money_integral' => ['type'=>'string','valid'=>'required', 'description'=>'支付积分', 'default'=>'', 'example'=>'234'],
    ...
    public function doPay($params)
    {
        ...
        try{
            ...
            foreach($trades['list'] as $val)
            {
                ...
                if(count($val['order'])>1||count($trades['list'])>1){
                    $bz=1;
                }
                if(!$result)
                {
        ...
        if($bz){
            $paymentBill['item_title'] = $paymentBill['item_title'].' 等多个商品';
        }
