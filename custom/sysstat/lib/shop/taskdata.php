...
class sysstat_shop_taskdata 
{
    ...
    public function getGoodsData($hotGoods,$params)
    {
        foreach ($hotGoods as $key => $value) {
            ...
            $goodsData[$value['item_id']]['amountintegral'] = $value['amountintegral'];
    ...
    public function getData($newTrade,$readyTrade,$readySendTrade,$alreadySendTrade,$completeTrade,$cancleTrade,$alreadyTrade,$params)
    {
        foreach ($newTrade as $key => $value)
        {
            ...
            $newTradeData[$value['shop_id']]['new_integral'] = $value['new_integral'];
        }
        foreach ($readyTrade as $key => $value)
        {
            ...
            $readyTradeData[$value['shop_id']]['ready_integral'] = $value['ready_integral'];
        }
        foreach ($readySendTrade as $key => $value)
        {
            ...
            $readySendTradeData[$value['shop_id']]['ready_send_integral'] = $value['ready_send_integral'];
        }
        foreach ($alreadySendTrade as $key => $value)
        {
            ...
            $alreadySendTradeData[$value['shop_id']]['already_send_integral'] = $value['already_send_integral'];
        }
        foreach ($completeTrade as $key => $value)
        {
            ...
            $completeTradeData[$value['shop_id']]['complete_integral'] = $value['complete_integral'];
        }
        foreach ($cancleTrade as $key => $value)
        {
            ...
            $cancleTradeData[$value['shop_id']]['cancle_integral'] = $value['cancle_integral'];
        }
        foreach ($alreadyTrade as $key => $value)
        {
            ...
            $alreadyTradeData[$value['shop_id']]['alreadyintegral'] = $value['alreadyintegral'];
        }
        foreach ($newTradeData as $key => $value)
        {
            ...
            $data[$value['shop_id']]['new_integral'] = $newTradeData[$value['shop_id']]['new_integral']?$newTradeData[$value['shop_id']]['new_integral']:0;
            ...
            $data[$value['shop_id']]['ready_integral'] =$readyTradeData[$value['shop_id']]['ready_integral']?$readyTradeData[$value['shop_id']]['ready_integral']:0;
            ...
            $data[$value['shop_id']]['ready_send_integral'] = $readySendTradeData[$value['shop_id']]['ready_send_integral']?$readySendTradeData[$value['shop_id']]['ready_send_integral']:0;
            ...
            $data[$value['shop_id']]['already_send_integral'] = $alreadySendTradeData[$value['shop_id']]['already_send_integral']?$alreadySendTradeData[$value['shop_id']]['already_send_integral']:0;
            ...
            $data[$value['shop_id']]['cancle_integral'] = $cancleTradeData[$value['shop_id']]['cancle_integral']?$cancleTradeData[$value['shop_id']]['cancle_integral']:0;
            ...
            //by zhangyan 2015-12-14 感觉原来写的方法有错误completeTradeData没有already_send_fee字段，也不该取complete_trade
            $data[$value['shop_id']]['complete_fee'] = $completeTradeData[$value['shop_id']]['complete_fee']?$completeTradeData[$value['shop_id']]['complete_fee']:0;
            $data[$value['shop_id']]['complete_integral'] = $completeTradeData[$value['shop_id']]['complete_integral']?$completeTradeData[$value['shop_id']]['complete_integral']:0;
            ...
            $data[$value['shop_id']]['alreadyintegral'] = $alreadyTradeData[$value['shop_id']]['alreadyintegral']?$alreadyTradeData[$value['shop_id']]['alreadyintegral']:0;
