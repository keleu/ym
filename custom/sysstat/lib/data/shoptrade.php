...
class sysstat_data_shoptrade
{
	...
	public function getTradeInfoList($filter=array())
	{
		...
		foreach ($tradeData as $key => $value) {
			if($value['new_trade']==0)
			{
				...
				$tradeData[$key]['averintegral'] = 0;
			}
			else
			{
				$tradeData[$key]['averPrice'] = number_format($value['new_fee'] / $value['new_trade'], 2, '.', '');
				$tradeData[$key]['averintegral'] = number_format($value['new_integral'] / $value['new_trade'], 0, '.', '');
			}
			$this->_blendshow($tradeData[$key]);
		}
		return $tradeData;
	}
	...
	public function getItemStaticsInfo($filter=array())
	{
		...
		foreach ($itemInfo as $key => $value)
		{
			foreach ($value as $k => $v)
			{
				...
				$result[$key]['amountintegral'] += $v['amountintegral'];
			}
		}
		...
		{
			//2016-3-31 by jianghui 获取积分+现金合计
			$value['amountblend'] = kernel::single('sysitem_blendShow')->totalshow($value['amountintegral'],$value['amountprice']);
			$itemOrder[]=$value;
		}
	...
	public function getItemCount($filter=array())
	{
		//$postFilter = $this->__check($filter);
		//$itemStaticMdl = app::get('sysstat')->model('item_statics');
		$itemData = $this->getItemStaticsInfo($filter);
	...
	{
		//2016-4-18 by jianghui 解决商家后台商品销售排行明细分页不对
		if($type == 'yesterday' || $type == 'beforday')
		{
			$filter['filter'] = $statFilter['commonday']['filter'];
		}
		elseif($type == 'selecttime')
		{
			$filter['filter'] = $statFilter['before']['filter'];
		}
		else
		{
			$filter['filter'] = $statFilter['filter'];
		}
		return $filter;
	}
	/**
	...
	public function getStatInfo($getData,$type=null)
	{
		if($type=='yesterday' || $type=='beforday')
		{
			...
			$commonday['averintegral'] = number_format($commonday['new_integral'] / $commonday['new_trade']);
			$this->_blendshow($commonday);
			...
			$beforeweek['averintegral'] = number_format($beforeweek['new_integral'] / $beforeweek['new_trade']);
			$this->_blendshow($beforeweek);
			$data = array(
				'commonday'=>$commonday,
				'beforeweek'=>$beforeweek,
			);
		...
		elseif($type=='selecttime')
		{
			...
			$before['averintegral'] = number_format($before['new_integral'] / $before['new_trade']);
			$this->_blendshow($before);
			...
			$after['averintegral'] = number_format($after['new_integral'] / $after['new_trade']);
			$this->_blendshow($after);
			$data = array(
				'before'=>$before,
				'after'=>$after,
			);
	...
	/**
	* ps ：获取积分+现金合计
	* Time：2016/03/31 10:19:54
	* @author jianghui
	*/
	function _blendshow(&$data){
        $data['new_blend'] = kernel::single('sysitem_blendShow')->totalshow($data['new_integral'],$data['new_fee']);

        $data['ready_blend'] = kernel::single('sysitem_blendShow')->totalshow($data['ready_integral'],$data['ready_fee']);

        $data['alreadyblend'] = kernel::single('sysitem_blendShow')->totalshow($data['alreadyintegral'],$data['alreadyfee']);

        $data['ready_send_blend'] = kernel::single('sysitem_blendShow')->totalshow($data['ready_send_integral'],$data['ready_send_fee']);

        $data['already_send_blend'] = kernel::single('sysitem_blendShow')->totalshow($data['already_send_integral'],$data['already_send_fee']);

        $data['cancle_blend'] = kernel::single('sysitem_blendShow')->totalshow($data['cancle_integral'],$data['cancle_fee']);

        $data['complete_blend'] = kernel::single('sysitem_blendShow')->totalshow($data['complete_integral'],$data['complete_fee']);

        $data['averBlend'] = kernel::single('sysitem_blendShow')->totalshow($data['averintegral'],$data['averPrice']);
	}
}
