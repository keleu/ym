...
class sysstat_operatorshop_task 
{
    ...
    public function getMemeberOrderInfo(array $params)
    {
        ...
        $qb->select('count(*) as userordernum ,sum(payment) as userfee,user_id as user_id,sum(payment_integral) as userintegral')
           ->from('systrade_trade')
    ...
    public function getTradeInfo(array $params) 
    {
        ...
        //新添加的订单数、额，
        $newTradeQb->select('count(*) as new_trade ,sum(payment) as new_fee ,trade_from as stats_trade_from,sum(payment_integral) as new_integral')
           ->from('systrade_trade')
        ...
        $completeTradeQb->select('count(*) as complete_trade ,sum(payment) as complete_fee ,trade_from as stats_trade_from,sum(payment_integral) as complete_integral')
           ->from('systrade_trade')
           ->where('status="TRADE_FINISHED"')
        ...
        $refundTradeQb->select('count(R.refund_id) as refunds_num ,sum(R.cur_money) as refunds_fee ,T.trade_from as stats_trade_from,sum(R.cur_integral) as refunds_integral')
        ...
        foreach ($type as $key )
        {
          ...
          $data[$key]['new_integral'] = $newTrade[$key]['new_integral']?$newTrade[$key]['new_integral']:0;
          ...
          $data[$key]['complete_integral'] = $completeTrade[$key]['complete_integral']?$completeTrade[$key]['complete_integral']:0;
          ...
          $data[$key]['refunds_integral'] = $refundTrade[$key]['refunds_integral']?$refundTrade[$key]['refunds_integral']:0;
        ...
        $data['all']['new_integral'] = $newTrade['pc']['new_integral']+$newTrade['wap']['new_integral'];
        ...
        $data['all']['complete_integral'] = $completeTrade['pc']['complete_integral']+$completeTrade['wap']['complete_integral'];
        ...
        $data['all']['refunds_integral'] = $refundTrade['pc']['refunds_integral']+$refundTrade['wap']['refunds_integral'];
    ...
    {
        $qb = app::get('systrade')->database()->createQueryBuilder();
        $qb->select('count(*) as shopaccountnum ,sum(payment) as shopaccountfee,shop_id as shop_id,sum(payment_integral) as shopaccountintegral')
           ->from('systrade_trade')
           ->where('status="TRADE_FINISHED"')
           ->andWhere('end_time>='.$qb->createPositionalParameter($params['time_start']))
           ->andWhere('end_time<'.$qb->createPositionalParameter($params['time_end']))
    ...
    public function getItemOrderInfo(array $params)
    {
        ...
        $qb->select('sum(R.num) as amountnum ,sum(R.payment) as amountprice,R.shop_id as shop_id,R.item_id as item_id,R.title as title,
                    R.pic_path as pic_path,I.cat_id as cat_id,sum(R.payment_integral) as amountintegral')
