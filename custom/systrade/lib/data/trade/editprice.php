...
class systrade_data_trade_editprice
{
    ...
    public function generate($params)
    {
        ...
        $tradeInfo = $objTrade->getTradeInfo('tid,status,total_fee,discount_fee,adjust_fee,post_fee,total_integral', array('tid'=>$params['tid']));//2016.4.14 by yangjie 添加应付积分total_integral
        ...
        foreach($tradeInfo['order'] as $k=>$v)
        {
            ...
            if($params['order'][$v['oid']])
            {
                ...
                //2016.4.14 by yangjie 添加判断条件total_integral，使金额和应付积分不能同时小于等于0
                if($aDataTmp['order'][$k]['payment']<=0 && $v['total_integral']<=0)
                {
