...
class systrade_data_trade_create{
   ...
   /**
    ...
    private function _chgdata(&$sdf, &$order_data, &$msg='',$aCart=array())
    {
        ...
        if($sdf['payment_type'] == "online" || $sdf['payment_type'] == "integral")
        {
        ...
        foreach ($aCart['resultCartData'] as $shop_id => $tval) {
            ...
            $order_data[$shop_id] = array(
                ...
                //2016-6-7 by jianghui 添加自提id
                'ziti_id'        => $sdf['ziti'][$shop_id]['ziti_id'],
            ...
            $order_data[$shop_id]['total_integral'] = $tval['cartCount']['total_integral'];
            $order_data[$shop_id]['payment_integral'] = $tval['cartCount']['total_integral'];
            // 用于生成促销日志表
            ...
            foreach($tval['object'] as $k =>$oval){
                $sub_stock = $sdf['payment_type'] == "offline" ? "1" : $oval['sub_stock'] ;
                ...
                $order_data[$shop_id]['order'][$k] = array(
                    ...
                    'integral'         => $oval['price']['integral'],
                    'num'              => $oval['quantity'],
                    'payment'          => $oval['price']['total_price'],
                    'total_fee'        => $oval['price']['total_price'],
                    'payment_integral' => $oval['price']['total_integral'],
                    'total_integral'   => $oval['price']['total_integral'],
                    'part_mjz_discount'=> $oval['price']['discount_price'],
