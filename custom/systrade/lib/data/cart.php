...
class systrade_data_cart {
    ...
    public function addCart($params)
    {
        ...
        //添加价格类型
        $data['typePrice']=$params['typePrice'];
    ...
    public function fastBuyStore($params)
    {
        ...
        $params['typePrice'] = input::get('typePrice')?input::get('typePrice'):0;
    ...
    public function getCartInfo($filter=array(), $needInvalid=true, $platform='pc')
    {
        ...
        //判断是否显示，这里调整 by li 已删除的商品在购物车中需要显示
        ...
        $shopNameArr = app::get('systrade')->rpcCall('shop.get.list',array('shop_id'=>$shopIds,'fields'=>'shop_id,shop_type,shop_name,is_ziti'));
    ...
    public function getBasicCartInfo($filter)
    {
        ...
        $data['skusData'] = $this->objLibItemInfo->getSkusList($skuIds,'sku_id,bn,item_id,spec_info,price,weight,status,integral,blend_integral,blend_price');
    ...
    private function __preCartInfo($data, $shopNameArr, $itemsData, $skusData, $needInvalid, $platform)
    {
        foreach( $data as $shopId=>$shopCartData )
        {
            ...
            if( $shopObjectData )
            {
                ...
                $resultCartData[$shopId]['is_ziti'] = $shopNameArr[$shopId]['is_ziti'];
        ...
        $return['totalCart'] = array(
            ...
            'totalAfterIntegral' => round(ecmath::number_minus( array($totalIntegral, 0) ),0),
        );
        //2016-3-30 by jianghui 添加积分+现金总计
        $return['totalCart']['totalBlend'] = kernel::single('sysitem_blendShow')->totalshow($totalIntegral,$totalPrice);
        $return['totalCart']['totalAfterBlend'] = kernel::single('sysitem_blendShow')->totalshow($return['totalCart']['totalAfterIntegral'],$return['totalCart']['totalAfterDiscount']);
        return $return;
    }
    ...
    private function __cartTotal(&$shopObjectData, $shop_id, $needInvalid, $platform)
    {
        ...
        foreach ($shopObjectData as $k1=>$v1)
        {
            ...
            if( $v1['valid'] && $v1['is_checked']=='1' )
            {
                ...
                $total_integral += $v1['price']['total_integral'];
            }
        }
        ...
        //2016-3-30 by jianghui 添加积分+现金和计
        $return['cartCount']['total_blend'] = kernel::single('sysitem_blendShow')->totalshow($total_integral,$total_fee);
        return $return;
    ...
    private function __preShopCartInfo($shopCartData, $itemsData, $skusData, $needInvalid, $platform)
    {
        ...
        foreach( $shopCartData as $row )
        {
            ...
            if($activityDetail['activity_price']>0)
            {
                ...
                //混合单价
                $shopObjectData[$k]['price']['blend'] = kernel::single('sysitem_blendShow')->totalshow(0,$activityDetail['activity_price']);
                $shopObjectData[$k]['price']['total_price'] = ecmath::number_multiple(array($activityDetail['activity_price'],$row['quantity'])); //购买此SKU总价格
                //购买此SKU总价格和总积分
                $shopObjectData[$k]['price']['total_blend'] = kernel::single('sysitem_blendShow')->totalshow(0,$shopObjectData[$k]['price']['total_price']);
            ...
            else
            {
                $temp_price=0;
                $temp_integral=0;
                //2016-1-15 by jiang 判断价格类型  typePrice  0:积分 1:积分加金额 2:金额
                switch ($row['typePrice']) {
                    case 0:
                        $temp_price=0;
                        $temp_integral=$skusData[$skuId]['integral'];
                        break;
                    case 1:
                        $temp_price=$skusData[$skuId]['blend_price'];
                        $temp_integral=$skusData[$skuId]['blend_integral'];
                        break;
                    default:
                        $temp_price=$skusData[$skuId]['price'];
                        $temp_integral=0;
                        break;
                }
                $shopObjectData[$k]['price']['price'] = $temp_price; //购买促销前单价
                $shopObjectData[$k]['price']['integral'] = $temp_integral;//积分单价
                //混合单价
                $shopObjectData[$k]['price']['blend'] = kernel::single('sysitem_blendShow')->totalshow($temp_integral,$temp_price);
                $shopObjectData[$k]['price']['typePrice'] = $row['typePrice'];
                $shopObjectData[$k]['price']['total_price'] = ecmath::number_multiple(array($temp_price,$row['quantity'])); //购买此SKU总价格
                $shopObjectData[$k]['price']['total_integral'] = round(ecmath::number_multiple(array($temp_integral,$row['quantity'])),0); //购买此SKU总积分

                //购买此SKU总价格和总积分
                $shopObjectData[$k]['price']['total_blend'] = kernel::single('sysitem_blendShow')->totalshow($shopObjectData[$k]['price']['total_integral'],$shopObjectData[$k]['price']['total_price']);
    ...
    private function applyFreepostage(&$shopObjectData, $vCartIds, $promotionDetail)
    {
        $forPromotionTotalPrice = 0; // 对应促销商品的总价
        $forPromotionTotalQuantity = 0; // 对应促销商品的总数量
        $forPromotionTotalIntegral = 0; // 对应促销商品的总积分
        ...
        foreach ($vCartIds as $cartId)
        {
            if($shopObjectData[$cartId]['valid'] && $shopObjectData[$cartId]['is_checked']=='1')
            {
                $forPromotionTotalPrice = ecmath::number_plus( array($forPromotionTotalPrice, $shopObjectData[$cartId]['price']['total_price']) );
                $forPromotionTotalQuantity = ecmath::number_plus( array($forPromotionTotalQuantity, $shopObjectData[$cartId]['quantity']) );
                $forPromotionTotalIntegral = ecmath::number_plus( array($forPromotionTotalIntegral, $shopObjectData[$cartId]['price']['total_integral']) );
        ...
        $applyData = array(
            ...
            'forPromotionTotalPrice' => $forPromotionTotalPrice,
            'forPromotionTotalQuantity' => $forPromotionTotalQuantity,
            'forPromotionTotalIntegral' => $forPromotionTotalIntegral,
        );
