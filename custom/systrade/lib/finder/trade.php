...
class systrade_finder_trade{
    ...
    public function __construct($app)
    {
        ...
        $this->tradeStatus1 = array(
            'WAIT_BUYER_PAY' => '已下单等待付款',
            'WAIT_SELLER_SEND_GOODS' => '未付款(货到付款)，待发',
            'WAIT_BUYER_CONFIRM_GOODS' => '已发货等待确认收货',
            'TRADE_FINISHED' => '已完成',
            'TRADE_CLOSED' => '已关闭',
            'TRADE_CLOSED_BY_SYSTEM' => '已关闭'
    ...
    public function detail_basic($tradeId)
    {
        ...
        $trade['status_des'] = $trade['pay_type'] == 'offline'? $this->tradeStatus1[$trade['status']]:$this->tradeStatus[$trade['status']];
        ...
        $pagedata['trade']['payment_blend'] = kernel::single('sysitem_blendShow')->totalshow($pagedata['trade']['payment_integral'],$pagedata['trade']['payment']);
        $pagedata['trade']['total_blend'] = kernel::single('sysitem_blendShow')->totalshow($pagedata['trade']['total_integral'],$pagedata['trade']['total_fee']);
        $pagedata['trade']['payed_blend'] = kernel::single('sysitem_blendShow')->totalshow($pagedata['trade']['payed_integral'],$pagedata['trade']['payed_fee']);
    ...
    public function detail_items($tradeId)
    {
        ...
        foreach ($orders as $key => $value) {
            $orders[$key]['total_blend'] = kernel::single('sysitem_blendShow')->totalshow($value['total_integral'],$value['total_fee']);
             $orders[$key]['blend'] = kernel::single('sysitem_blendShow')->totalshow($value['integral'],$value['price']);
        }
    ...
    public function detail_other($tradeId)
    {
        ...
        //by zhangyan 2015-11-25 shop_demo没有数据，要换成trade_memo
        $rows ="shop_flag,shop_memo,trade_from,ip,trade_from,modified_time,trade_memo";
    ...
    public $column_status = '订单状态';

    public function column_edit(&$colList, $list)
    {
        foreach($list as $k=>$row)
        {
            if(in_array($row['status'],['TRADE_FINISHED','TRADE_CLOSED','TRADE_CLOSED_BY_SYSTEM']))
            {
                $colList[$k] = "";
            }
            else
   ...
   public function column_status(&$colList, $list)
    {
        foreach($list as $k=>$row)
        {
            $colList[$k] =$row['pay_type'] == 'offline'? $this->tradeStatus1[$row['status']]:$this->tradeStatus[$row['status']];
        }
    }
}
