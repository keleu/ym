...
class systrade_getTradeData {
    ...
    public function getTradeInfo($fields, $tid, $oid = null, $filter)
    {
        ...
        $tradeInfo['payment_blend'] = kernel::single('sysitem_blendShow')->totalshow($tradeInfo['payment_integral'],$tradeInfo['payment']);
        //商品总额
        $tradeInfo['total_blend'] = kernel::single('sysitem_blendShow')->totalshow($tradeInfo['payment_integral'],$tradeInfo['total_fee']);
        // dump($tradeInfo);exit;
        ...
        foreach( $orders as $k=>$value )
        {
            ...
            $orders[$k]['blend'] = kernel::single('sysitem_blendShow')->totalshow($value['integral'],$value['price']);
            //总计
            $orders[$k]['total_blend'] = kernel::single('sysitem_blendShow')->totalshow($value['total_integral'],$value['total_fee']);
            //应付
            $orders[$k]['payment_blend'] = kernel::single('sysitem_blendShow')->totalshow($value['payment_integral'],$value['payment']);
        }
