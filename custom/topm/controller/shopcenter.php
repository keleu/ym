    ...
    private function __getShowItems($data)
    {
        ...
        foreach ($data as $key => $value)
        {
            if($value['params']['isstart'])
            {
                ...
                //首页商品展示排序，by liuxin，2015-12-16
                $item_sort = substr($value['params']['ordersort'],0,-2);
                $item_sort_mode = substr($value['params']['ordersort'],-1);
                if($item_sort_mode == 's'){
                    $itemsList['list'] = $this->array_sort($itemsList['list'],$item_sort,'asc');
                }
                elseif($item_sort_mode == 'l'){
                    $itemsList['list'] = $this->array_sort($itemsList['list'],$item_sort,'desc');
                }
