...
class topm_ctl_cart extends topm_controller{
    ...
    public function add()
    {
        ...
        //增加价格类型，需改进，by liuxin，2015-12-08
    ...
    public function total()
    {
        ...
        foreach ($cartInfo['resultCartData'] as $shop_id => $tval) {
            ...
            $trade_data['allPaymentIntegral'] = $objMath->number_plus(array($trade_data['allPaymentIntegral'] ,$totalInfo['total_integral']));
            ...
            $trade_data['shop'][$shop_id]['total_integral'] = $totalInfo['total_integral'];
            ...
            //2016-3-30 by jianghui 添加积分加现金的总价
            $trade_data['shop'][$shop_id]['total_blend'] = kernel::single('sysitem_blendShow')->totalshow($totalInfo['total_integral'],$totalInfo['total_fee']);
            $trade_data['shop'][$shop_id]['payment_blend'] = kernel::single('sysitem_blendShow')->totalshow($totalInfo['payment_integral'],$totalInfo['payment']);
        }
    ...
    public function checkout()
    {
        ...
        try
        {
            ...
            if(isset($postData['pay_type']))
            {
                ...
                $pagedata['payType'] = array('pay_type'=>'online','name'=>'线上支付');
            }
            ...
            if($ziti_shopid && $pagedata['def_addr'])
            {
                $area = explode(':',$pagedata['def_addr']['area']);
                $area = implode(',',explode('/',$area[1]));
                foreach ($ziti_shopid as $v_ziti) {
                    $zitiData[$v_ziti] = app::get('topm')->rpcCall('logistics.ziti.list',array('area_id'=>$area,'shop_id'=>$v_ziti));
                }
                $pagedata['zitiDataList'] = $zitiData;
            }

            $shop_ids = array_keys($pagedata['cartInfo']['resultCartData']);
            // dump($pagedata['zitiDataList']);exit;
            if( $ziti_shopid )
            {
                $pagedata['dtyList'] = $this->__getDtyList($shop_ids,$ziti_shopid,$zitiData);
            }
            else
            {
            ...
            //地址
            $pagedata['next_checkout'] = url::action("topm_ctl_cart@checkout",$postData);

            //2016-3-9 by jiang 查找会员是否绑定手机号码
            $userId = userAuth::id();
        ...
        $pagedata['ifOpenOffline'] = $pagedata['cartInfo']['totalCart']['totalAfterIntegral']>0?'false':$pagedata['ifOpenOffline'];//2016.4.19 by yangjie 当有积分的时候不显示货到付款
    ...
    public function getPayTypeList()
    {
        ...
        $pagedata['ifOpenOffline'] = input::get('isoffline');//2016.4.19 by yangjie 获取传过来的isoffline值
    ...
    */
    public function consultMap(){
        //获取自提信息
        $pagedata = app::get('topm')->rpcCall('logistics.ziti.get',array(
            'id'=>$_GET['ziti_id']
        ));
        // dump($pagedata);exit;
