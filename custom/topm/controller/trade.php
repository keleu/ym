...
class topm_ctl_trade extends topm_controller{
    ...
    public function create()
    {
        ...
        if( $postData['shipping'])
        {
            ...
            {
                //验证店铺类型
                $shopdata = app::get('topm')->rpcCall('shop.get.detail',array('shop_id'=>$k,'fields'=>'shop_type,is_ziti'))['shop'];
                ...
                if($v['template_id'] == 0)
                {
                    ...
                    //2016-5-7 by jianghui 添加自提id
                    $ziti .= $k.":".$zitiAddr['area'].$zitiAddr['addr'].'='.$zitiAddr['id'].";";
                }
        ...
        try{
            //by张艳2015-12-09 添加积分支付方式
            if($postData['payment_type'] == "online" || $postData['payment_type'] =='integral' )
            {
