...
class topm_ctl_collect extends topm_controller{
    ...
    */
    ...
    /**
    * ps ：商品收藏不需要提示信息
    * Time：2016/03/17 09:35:58
    * @author jianghui
    */
    function ajaxFavNomess() {
        $userId = userAuth::id();
        if(!$userId)
        {
            return redirect::action('topm_ctl_passport@signin');
        ...
        if (!app::get('topm')->rpcCall('user.itemcollect.add', $params))
        {
            return $this->splashNomess('error',null, app::get('topm')->_('该商品已经收藏！'));
        }
        else
        {
           return  $this->splashNomess('success',null,app::get('topm')->_('商品收藏添加成功'));
        }
    }
    ...
    */
    ...
    /**
    * ps ：商品删除收藏不需要提示信息
    * Time：2016/03/17 09:38:24
    * @author jianghui
    */
        
    function ajaxFavDelNomess()
    {
        $userId = userAuth::id();
        if(!$userId)
        {
        ...
        if(empty($params['item_id']))
        {
            return $this->splashNomess('error',null, app::get('topm')->_('商品id不能为空！'));
        }

        if (!app::get('topm')->rpcCall('user.itemcollect.del', $params))
        {
            return $this->splashNomess('error',null, app::get('topm')->_('商品收藏删除失败！'));
        }
        else
        {
           return  $this->splashNomess('success',null,app::get('topm')->_('商品收藏删除成功'));
        }
    }
