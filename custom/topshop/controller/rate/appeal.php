...
class topshop_ctl_rate_appeal extends topshop_controller {
    ...
    public function appeaInfo()
    {
        ...
        $fields = 'rate_id,item_id,item_title,item_pic,item_price,result,content,rate_pic,created_time,is_appeal,appeal_status,appeal.appeal_id,appeal.content,appeal.appeal_type,appeal.evidence_pic,appeal.reject_reason,appeal.appeal_log,item_integral';
        ...
        //2016-3-31 by jianghui 价格显示积分+现金
        $data['item_blend'] = kernel::single('sysitem_blendShow')->totalshow($data['item_integral'],$data['item_price']);
        $pagedata['rate'] = $data;
    ...
    private function __searchAppealData()
    {
        ...
        $fields = 'rate_id,tid,item_title,result,item_id,item_price,user_id,appeal_status,appeal_again,appeal_time,appeal.content,appeal.rate_id,appeal.evidence_pic,item_integral';
        $params = ['role'=>'seller','page_no'=>$current,'page_size'=>$this->limit,'fields'=>$fields];
        ...
        foreach( $pagedata['rate'] as $k=>$row)
        {
            ...
            //2016-3-31 by jianghui 价格显示积分+现金
            $pagedata['rate'][$k]['item_blend'] = kernel::single('sysitem_blendShow')->totalshow($row['item_integral'],$row['item_price']);
        }
