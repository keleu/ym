...
class sysaftersales_ctl_refunds extends desktop_controller {
    ...
    public function refundsPay($aftersalesBn)
    {
        ...
        //2016-3-30 by jianghui 添加积分+现金总计
            $data['total_blend'] = kernel::single('sysitem_blendShow')->totalshow($data['total_integral'],$data['total_price']);
    ...
    public function dorefund()
    {
        ...
        $refundsData = json_decode($postdata['refundsData'],true);
        $userId = $refundsData['user_id'];
        
        kernel::single('weixin_base')->jfCound_change(
            $userId,
            '积分退款',
            (int)$refundsData['money_integral']
        );
