...
class sysaftersales_data {
    ...
    public $aftersalesSkuFields = 'oid,sku_id,item_id,title,pic_path,spec_nature_info,price,payment,aftersales_status,complaints_status,integral,payment_integral';//添加积分字段
    ...
    public function getAftersalesList($fields, $filter, $page=1, $limit, $orderBy="created_time desc")
    {
        ...
        if( $aftersalesList && isset($fields['extends']['sku']) )
        {
            ...
            foreach ($skuData as $a => $b) {
                //2016-3-30 by jianghui 添加积分+现金总计
                $skuData[$a]['blend'] = kernel::single('sysitem_blendShow')->totalshow($b['integral'],$b['price']);
            }
