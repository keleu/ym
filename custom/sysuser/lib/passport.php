...
class sysuser_passport
{
    ...
    public function signupUser($data)
    {
        ...
        //2016-1-28 by jiang 如果是app添加会员则 必须要有用户名
        if($data['account_id']){
            $accountUser = $this->__preAccountUser1($data);
        }else{
    ...
    private function __preAccountUser($data)
    {
        ...
        //by 张艳 2015-12-07 如果用会员名称注册，也要保存手机号码
        if ($type == 'login_account') {
           $account['mobile']=trim($data['mobile']);
        }
    ...
    /**
    * ps ：用户名必填
    * Time：2016/01/28 12:13:20
    * @author jiang
    */
     private function __preAccountUser1($data)
    {
        $type = kernel::single('pam_tools')->checkLoginNameType($data['account']);
        $type='login_account';
        //检查注册的账号是否合法
    ...
    {
        $user['user_id'] = $userId;
        //by 张艳 2015-12-07 保存会员是谁 发展来的
        $user['agent_account'] = $data['account_id'];
        $user['reg_ip'] = $data['reg_ip'];
        $user['regtime'] = time();
        $user['opend_id'] = $data['opend_id'];
        return $user;
    }
    ...
    /**
     * 验证用户信息是否合法
     * 和checkSignupAccount方法一样，把throw改成return，因为有些时候需要返回错误信息，
     * 不是直接结束代码
     * Time：2016/05/13 17:50:02
     * @author li
     * @param 参数类型
     * @return 返回值类型
    */
    public function checkSignupAccount_info($account,$type){
        if( empty($account) )
        {
        ...
        switch( $type )
        {
            case 'login_account':
                if( strlen(trim($account))< 4 )
                {
                    return ['success'=>false,'msg'=>'登录账号最少4个字符'];
                }
                elseif( strlen($account) > 30 )
                {
                    return ['success'=>false,'msg'=>'登录账号过长，请换一个重试'];
                }

                if(!preg_match('/^[^\x00-\x2d^\x2f^\x3a-\x3f]+$/i', trim($account)) )
                {
                    return ['success'=>false,'msg'=>'该登录账号包含非法字符'];
                }
            ...
            case 'email':
                if(!preg_match('/^(?:[a-z\d]+[_\-\+\.]?)*[a-z\d]+@(?:([a-z\d]+\-?)*[a-z\d]+\.)+([a-z]{2,})+$/i',trim($account)) )
                {
                    return ['success'=>false,'msg'=>'邮件格式不正确'];
                }
                $message = $this->app->_('该邮箱已被注册，请更换一个');
                break;
            case 'mobile':
