...
class sysuser_api_getItemCollectList {
    ...
    private function __itemData($data)
    {
        foreach ($data as $key => $value)
        {
            ...
            $collectData[$value['item_id']]['goods_point'] = $value['goods_point'];
        ...
        $itemData = $objMdlItem->getList('item_id,integral,price,blend_integral,blend_price',array('item_id'=>$collectItemId));
        $itemList=array();
        foreach ($itemData as $key => $value)
        {
            $itemId[$key] = $value['item_id'];
            $itemList[$value['item_id']] = $value;
        }
        foreach ($collectItemId as $value)
        {
            ...
            //2016-3-30 by jianghui 价格直接从商品表里面取
            $collectData[$value]['price'] = $itemList[$value]['price'];
            $collectData[$value]['integral'] = $itemList[$value]['integral'];
            $collectData[$value]['blend'] = kernel::single('sysitem_blendShow')->show($itemList[$value]['blend_integral'],$itemList[$value]['blend_price']);
        }
        return $collectData;
