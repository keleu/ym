...
class base_validator_validator
{
    ...
    /**
     * The dataDesc under validation.
     *
     * @var array
     */
    protected $dataDesc = array();
    ...
    public function openDescModel(array $dataDesc = array()){
        $this->dataDesc = $dataDesc;
    }

    protected function getDesc($attribute){
       return array_key_exists($attribute, $this->dataDesc)?'"'.$this->dataDesc[$attribute].'"':$attribute;;
    }
    ...
    public function passes()
    {
        ...
        foreach ($this->rules as $attribute => $rules)
        {
            foreach ($rules as $key =>$rule)
            {
                if($describe)
                {
                    $this->validate($this->getDesc($attribute), $rule,$describe[$attribute][$key]);
                }
                else
                {
    ...
    protected function doReplacements($message, $attribute, $rule, $parameters,$describe)
    {
        if(empty($describe))
        {
            $message = str_replace(':attribute', $this->getDesc($attribute) , $message);
        }
        else
        {
            $message = $describe;
