...
class syspromotion_freepostage extends syspromotion_abstract_promotions {
    ...
    private function __saveFreepostageItem(&$freepostageData, $promotion_id)
    {
        ...
        $searchParams = array(
            ...
            'fields' => 'item_id,title,image_default_id,cat_id,brand_id,price,integral,blend_price,blend_integral',
            //2016-5-13 添加不分页判断
            'no_page' => 1
        );
        ...
        foreach($freepostageData['rel_item_ids'] as $itemid)
        {
            $freepostageRelationItem = array(
                ...
                //添加积分
                'price' => $newItemsList[$itemid]['price'],
                'integral' => $newItemsList[$itemid]['integral'],
                'blend_price' => $newItemsList[$itemid]['blend_price'],
                'blend_integral' => $newItemsList[$itemid]['blend_integral'],
    ...
    private function __preareData($data) {
        ...
        if( !in_array($data['condition_type'], array('money', 'quantity','point')) )
        {
        ...
        if( $data['condition_type'] == 'point' )
        {
            if($data['limit_point']<=0)
            {
                throw new \LogicException('积分必须大于0');
            }
            unset($aResult['limit_money']);
            unset($aResult['limit_quantity']);
        }
        //2016-3-18 by jiang  满减开始时间可以是当天 好测试
