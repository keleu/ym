...
class topc_ctl_item extends topc_controller {
    ...
    public function index()
    {
        ...
        //未上架商品，无法浏览 2015-12-18 BY qianjin
        ...
        if($promotionInfo)
        {
            $userInfo = app::get('topshop')->rpcCall('user.grade.fullinfo');
            foreach($promotionInfo as $vp)
            {
                $basicPromotionInfo = app::get('topc')->rpcCall('promotion.promotion.get', array('promotion_id'=>$vp['promotion_id'], 'platform'=>'pc','has_detail'=>true));
                //2016-3-25 by jianghui 判断该用户是否能享受该优惠
                $valid_grade = explode(',', $basicPromotionInfo['detail']['valid_grade']);
                if( in_array($userInfo['userlist']['grade_id'], $valid_grade) ){
        ...
        //2015-11-26 by jiang 添加服务承诺字样 程序不做限制
        $tuihuoRate = app::get('sysconf')->getConf('site.tuihuo');
        $huanhuoRate = app::get('sysconf')->getConf('site.huanhuo');
        $pagedata['tuihuoRate']=$tuihuoRate?$tuihuoRate:'七天包退';
        $pagedata['huanhuoRate']=$huanhuoRate?$huanhuoRate:'十五天包换';
        ...
        //2016-4-7 by jianghui 添加看了又看的数据
        $params=array(
    ...
    private function __getShopDsr($shopId)
    {
        ...
        $params['catDsrDiff'] = true;//2016.4.29 by yangjie 把false改成true开启返回行业平均分同比
