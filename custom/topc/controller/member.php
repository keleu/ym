...
class topc_ctl_member extends topc_controller {
    ...
    public function index()
    {
        ...
        $tradeParams['fields'] = 'tid,shop_id,user_id,status,payment,total_fee,post_fee,payed_fee,receiver_name,created_time,receiver_mobile,discount_fee,need_invoice,adjust_fee,order.title,order.price,order.num,order.pic_path,order.tid,order.oid,order.spec_nature_info,order.item_id,activity,order.integral,total_integral,payment_integral,pay_type';//2016.4.19 by yangjie 添加pay_type
        ...
        $collectParams['fields'] = "gnotify_id,image_default_id,goods_name,goods_price,goods_point,item_id,user_id,cat_id,object_type";
    ...
    {
        $postData = utils::_filter_input(input::get());
        //获取请求链接中的verifyType参数，用于后续判断,2015-12-04,by liuxin
        $firstId = strpos($_SERVER['HTTP_REFERER'],'verifyType=') + 11;
        $secondId = (int)strpos($_SERVER['HTTP_REFERER'],'&',$firstId);
        $postData['verifyType'] = substr($_SERVER['HTTP_REFERER'],$firstId,($secondId-$firstId));
        //通过解析 URL 的 type 区分是哪个界面调用这个方法 2015-12-25 by qianjin
        $Type=strpos($_SERVER['HTTP_REFERER'],'type')+5;
        $kind=substr($_SERVER['HTTP_REFERER'],$Type);
        ...
        //增加了 绑定手机号 是否为“修改”的判断 2015-12-25 By qianjin
        if($kind=="update"){
