...
class topc_ctl_shopcenter extends topc_controller
{
    ...
    public function index()
    {
        ...
        //店铺商品
        $items = shopWidgets::getWidgetsDataOrder('showitems',$shopId,'0,1');
        ...
        //未上架商品，在店铺首页不能显示 2015-12-18 by qianjin
        foreach ($items as $key => $value) {
            foreach ($value as $key1 => $value1) {
                $params['item_id'] = $value1[item_id];
                $params['fields'] = "approve_status";
                $detailData = app::get('topc')->rpcCall('item.get',$params);
                if($detailData[approve_status]!='onsale'){
                    unset($pagedata[items][$key][$key1]);
                }
            }
        }

        //2016-3-15 by jianghui 查找商品是否已经收藏过该店铺
        $user_id = userAuth::id();
        $retshop = app::get('sysuser')->model('shop_fav')->getRow('snotify_id',array('shop_id'=>$pagedata['shopdata']['shop_id'],'user_id'=>$user_id));
    ...
    public function search()
    {
        ...
        //增加搜索限制条件 2016年2月23日 10:23:05 by shen
        ...
        //增加搜索关键字为title 2016年2月21日 13:39:53 by shen
        if(isset($params['search_keywords'])){
            $searchParams['search_keywords'] = $params['search_keywords'];
        }else{
            $searchParams['search_keywords'] = $params['title'];
        }
        ...
        //未上架商品，在店铺中不能显示 2015-12-18 by qianjin
        foreach ($items as $key => $value) {
            $params['item_id'] = $key;
            $params['fields'] = "approve_status";
            $detailData = app::get('topc')->rpcCall('item.get',$params);
            if($detailData[approve_status]=='instock'){
                unset($pagedata[items][$key]);
            }
        }
