...
EOF;
    ...
    function image_lib($tag='',$page=1){
        $pagelimit = 12;
        $otag_rel = app::get('desktop')->model('tag_rel');
        $oimage = $this->app->model('images');

        $ret_tag = $otag_rel->getList('rel_id',array('tag_type'=>'image','tag_id'=>$tag));
        $ret_tag = array_column($ret_tag,'rel_id');
        $filter = array();
        if($tag){
            $filter = array('id|in'=>$ret_tag);
        }
    ...
    /**
    * ps ：查找图片是否存在
    * Time：2016/03/17 13:17:14
    * @author jianghui
    */
    function is_exist(){
        $data=$_POST['src'];
        $data=explode('images', $data);
        $url=MEDIA_DIR.$data[1];
        //该图片已存在
        if( file_exists($url) ){
         echo json_encode(array('success'=>true,'data'=>'ok'));exit;
        }else{
            echo json_encode(array('success'=>true,'data'=>'no'));exit;
        }
    }
